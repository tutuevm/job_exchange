...
async def auth_user_issue_jwt(
    manager: UserManagerDependence, user_data: UserLoginSchema, uow: UOWDependence
):
    result = await AuthService().auth_user_issue_jwt(
        uow=uow, manager=manager, user=user_data
    )
    return result


@auth_router.post("/check_jwt")
async def check_jwt(manager: UserManagerDependence, jwt: str):
    try:
...
async def check_user(uow: UOWDependence, payload: UserPayloadDependence):
...
async def refresh_access_token(
    ...
    return await AuthService().refresh_auth_cuurent_user(
        manager=manager, payload=payload
    )
