...
class JobService:
    ...
    async def accept_responded_user(self, uow: InterfaceUnitOfWork, user_id: UUID, job_id: UUID, current_user):
        async with uow:
            ...
            job_response = await uow.user_job_association.get_association(user_id=user_id, job_id=job_id)
            if job_response.response_status != JobResponseType.SUBMITTED:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Invalid response status. Expected - {JobResponseType.SUBMITTED}, arrived - {job_response.response_status}' )
    ...
    async def set_compete_status_job(self, uow: InterfaceUnitOfWork, user_id: UUID, job_id: UUID, current_user):
        async with uow:
            job_response = await uow.user_job_association.get_association(user_id=user_id, job_id=job_id)
            job = await uow.job.find_by_filter(id=job_id)
    ...
    async def accept_and_close_job(self, uow: InterfaceUnitOfWork, job_id: UUID, current_user):
        async with uow:
            job = await uow.job.find_by_filter(id=job_id)
